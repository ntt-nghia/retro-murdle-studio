...
import type { Solution, Suspect, Weapon, Location, MurdleData } from "@/lib/types";
...
export default function SolutionModal({ isOpen, onClose, title, solution, story, items, isFinalAccusation, onAccuse, status }: SolutionModalProps) {
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select onValueChange={(v) => setAccusation(a => ({ ...a, suspect: v }))}>
              ...
              <SelectContent className="font-code bg-black border-primary text-primary retro-frame-inset">{items.suspects.map(s => <SelectItem key={s.name} value={s.name}>{s.name}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
            ...
            <Select onValueChange={(v) => setAccusation(a => ({ ...a, weapon: v }))}>
              ...
              <SelectContent className="font-code bg-black border-primary text-primary retro-frame-inset">{items.weapons.map(w => <SelectItem key={w.name} value={w.name}>{w.name}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
            ...
            <Select onValueChange={(v) => setAccusation(a => ({ ...a, location: v }))}>
              ...
              <SelectContent className="font-code bg-black border-primary text-primary retro-frame-inset">{items.locations.map(l => <SelectItem key={l.name} value={l.name}>{l.name}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <Button onClick={handleAccuseClick} disabled={!accusation.suspect || !accusation.weapon || !accusation.location} className="retro-button w-full !bg-destructive mt-4">CONFIRM ACCUSATION</Button>
        </div>
      );
    }
